...
1. まず、このページで[チケット種別の全体像](#まず押さえること-チケット種別の全体マップ)を掴みます。
2. 次に、[新機能開発と不具合対応の基本線](#段階的な具体化と種別ごとの処理)を確認します。
3. そのあと、[種別ごとのステータス遷移](#チケット種別ごとのステータス遷移)から必要なワークフローへ進みます。
4. 詳細が必要な場合は、以下の資料を参照します。
    - [[チケット管理の流れ]] - 機能開発・改善と不具合対応のフロー図と実例
    - [[成果物ごとの役割]] - 各成果物の使いどころと設計レビューの観点
    - [[判断に迷ったときのFAQと実例]] - よくある判断ポイントと規模別の使い分け
    - [[ワークフロー]] - 全ステータス一覧と種別ごとの遷移

基本方針
-------------------------

- 思いついた内容は、粗い段階でもまず記録します。
- 機能開発・改善は`アイデア → 要望 → タスク`の順に具体化します。
...
| 規模   | 目安      | 作業プラン           | チェックリスト           |
| ------ | --------- | -------------------- | ------------------------ |
| 小規模 | 半日以内  | 判断が必要な場合のみ | 品質基準が複雑な場合のみ |
| 中規模 | 半日〜2日 | 推奨                 | 推奨                     |
| 大規模 | 3日以上   | 必須                 | 必須                     |

詳細は[FAQの規模別使い分け](faq.md#プロジェクト規模別の使い分け) - 詳細な規模別の判断基準を参照してください。
...
機能開発・改善は、まずアイデアとして記録し、要望として整理したうえで、必要な情報が揃った段階で着手可能なタスクに昇格します。
...
詳細なフロー図や具体例は、[[チケット管理の流れ]] - アイデアから実装完了までの流れを図表と実例で解説を参照してください。
...
詳細な考え方は、[[成果物ごとの役割]] - 各成果物の使いどころと相互参照の考え方を参照してください。

| 種別           | 主な目的                   | 参照先                                                      |
| -------------- | -------------------------- | ----------------------------------------------------------- |
| アイデア       | ひらめきや仮説を残す       | [[成果物ごとの役割]] - 使いどころと設計レビュー |
| 要望           | 実現したい内容を明確にする | [[成果物ごとの役割]] - 使いどころと設計レビュー |
| 不具合         | 瑕疵や異常を記録する       | [[成果物ごとの役割]] - 使いどころと設計レビュー |
| タスク         | 着手可能な作業に落とし込む | [[成果物ごとの役割]] - 使いどころと設計レビュー |
| 作業プラン     | 実装手順と判断を記録する   | [[成果物ごとの役割]] - 使いどころと設計レビュー |
| チェックリスト | 品質基準を確認する         | [[成果物ごとの役割]] - 使いどころと設計レビュー |
